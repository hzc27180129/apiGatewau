...
local BasePlugin = require("apiGateway.plugins.base_plugin")
...
function LimitIpHandler:access()
	...
	local key = "limit:ip:blacklist";--存放在redis的黑名单ip列表
	local user_ip = utils.get_ip();
	local shared_ip_blacklist = ngx.shared.shared_ip_blacklist;--nginx的本地缓存
	--获得本地缓存的最新刷新时间
	local last_update_time = shared_ip_blacklist:get("last_update_time");
	
	if last_update_time ~= nil then --获取本地缓存的黑名单ip，如果有则判断缓存是否过期，如果没过期就判断当前请求的ip是否在黑名单中
	...
	--如果本地缓存中没有数据，或缓存已过期。则到redis中取数据
	local red = redis:new()  --创建一个对象，注意是用冒号调用的
	--设置超时（毫秒）  
	red:set_timeout(1000)  				---这些可以放到配置文件中
	--建立连接  
	local redisHost = "172.18.0.2"  ---这些可以放到配置文件中
